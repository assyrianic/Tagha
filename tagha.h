 ...
 * - having named functions and global values for easy access from host application and dynamic loading.
 * - concise and minimal so that we can operate on embedded systems as well.
 * 
 * ------------------------------ start of header ------------------------------
 ...
 * n bytes: func table
 *		1 byte: 0 if bytecode func, 1+ if it's a system call.
 * 		4 bytes: string size + '\0' of func string
 * 		n bytes: func string
 * 		if bytecode func: 8 bytes: code segment offset.
 ...
 * 
 * .code segment - n bytes: instruction stream.
 */
 ...
 * If 8 params, Thaw will hold the 8th param, Semkath the first.
 * 
