...
static jitfunc_ptr jitfunc_create(void)
{
# ifdef OS_WINDOWS
...
static void jitfunc_protect(jitfunc_ptr jit)
{
	if( !jit )
		return;
# ifdef OS_WINDOWS
...
static void jitfunc_free(jitfunc_ptr *jitref)
{
	if( !jitref || !*jitref )
		return;
# ifdef OS_WINDOWS
...
static void jitfunc_insert(jitfunc_ptr jit, const size_t bytes, const uint64_t instr)
{
	if( !jit )
		return;
...
static void jitfunc_imm(jitfunc_ptr jit, const uint32_t imm)
{
	if( !jit )
		return;
...
#undef INSTR_SET
	...
	exec_pushi: { /* char: opcode | i64: imm */
		...
		jitfunc_insert(jit, 1, 0x68); jitfunc_imm(jit, conv.l); // push lower half of int64
		jitfunc_insert(jit, 1, 0x68); jitfunc_imm(jit, conv.h); // push upper half of int64
		DISPATCH();
	}
	...
	exec_movi: { /* char: opcode | char: regid | i64: imm */
		...
		// split int64 between eax and edx.
		
		// mov eax, imm32
		jitfunc_insert(jit, 1, 0xb8); jitfunc_imm(jit, conv.l);
		// mov edx, imm32
		jitfunc_insert(jit, 1, 0xba); jitfunc_imm(jit, conv.h);
		DISPATCH();
	}
	exec_mov: { /* char: opcode | char: dest reg | char: src reg */
		const uint16_t regids = *pc.PtrUInt16++;
		jitfunc_insert(jit, 1, 0xb8);
