...
static NO_NULL HOT void _tagha_module_exec(union TaghaPtr ip, struct TaghaModule *module);
...
static bool _tagha_module_start(struct TaghaModule *const module, const struct TaghaItem *const func, const size_t args, const union TaghaVal params[const], union TaghaVal *const restrict retval)
{
	if( func->flags & TAGHA_FLAG_NATIVE ) {
		...
		if( module->osp - bytes < module->opstack ) {
			...
			module->lr = NIL;
			_tagha_module_exec(( union TaghaPtr )(( const void* )(func->item)), module);
			if( retval != NULL ) {
...
static void _tagha_module_exec(union TaghaPtr ip, struct TaghaModule *const restrict vm)
{
#if (defined(COMPILER_CLANG) || defined(COMPILER_GCC))
#	if defined(PLATFORM_AMD64)
	register uintptr_t vreg_osp __asm__ ("r15") = vm->osp;
#	elif defined(PLATFORM_ARM64)
	register uintptr_t vreg_osp __asm__ ("x15") = vm->osp;
#	elif defined(PLATFORM_RISCV64)
	register uintptr_t vreg_osp __asm__ ("x31") = vm->osp;
#	endif 
#else
	uintptr_t vreg_osp = vm->osp;
#endif

#if (defined(COMPILER_CLANG) || defined(COMPILER_GCC))
#	if defined(PLATFORM_AMD64)
	register const struct TaghaSymTable *table_funcs __asm__ ("r14") = vm->funcs;
	register const struct TaghaSymTable *table_vars  __asm__ ("r13") = vm->vars;
#	elif defined(PLATFORM_ARM64)
	register const struct TaghaSymTable *table_funcs __asm__ ("x14") = vm->funcs;
	register const struct TaghaSymTable *table_vars  __asm__ ("x13") = vm->vars;
#	elif defined(PLATFORM_RISCV64)
	register const struct TaghaSymTable *table_funcs __asm__ ("x30") = vm->funcs;
	register const struct TaghaSymTable *table_vars  __asm__ ("x29") = vm->vars;
#	endif
#else
	const struct TaghaSymTable *table_funcs = vm->funcs;
	const struct TaghaSymTable *table_vars  = vm->vars;
#endif
	
	const uintptr_t lower_seg     = vm->low_seg;
	const uintptr_t mem_bnds_diff = vm->high_seg - lower_seg;
...
#	define DBG_JMP \
		do { \
			const uint_fast8_t instr = *ip.uint8++; \
			if( instr>=MaxOps ) { \
				printf("instr : '%#x' '%u' | offset: %zu\n", instr, instr, ( uintptr_t )(ip.uint8) - vm->script); \
...
#	define FAST_JMP      goto *dispatch[*ip.uint8++]
	
#	define DISPATCH()    FAST_JMP
	...
	exec_movi: { /// u8: opcode | u8: dest reg | u64: imm
		const uint_fast8_t regid = *ip.uint8++;
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		rsp[regid] = *ip.val++;
		DISPATCH();
	}
	exec_mov: { /// u8: opcode | u8: dest reg | u8: src reg
		const uint_fast16_t instr = *ip.uint16++;
	...
	/// make room via opstack pointer to create live registers.
	exec_alloc: { /// u8: opcode | u8: cells
		const uint_fast8_t cells = *ip.uint8++; /// allocs up to 8kb per stack frame.
		const size_t alloc_size  = sizeof(union TaghaVal) * cells;
		if( vreg_osp - alloc_size < vm->opstack ) {
	...
	/// reduce live registers.
	exec_redux: { /// u8: opcode | u8: cells
		const uint_fast8_t cells  = *ip.uint8++;
		const size_t dealloc_size = sizeof(union TaghaVal) * cells;
		if( vreg_osp + dealloc_size > vm->opstack + vm->opstack_size ) {
			vreg_osp = vm->opstack + vm->opstack_size;
			DISPATCH();
		} else {
			vreg_osp += dealloc_size;
			DISPATCH();
		}
	}
	...
	exec_lra: { /// u8: opcode | u8: regid | u16: offset
		const uint_fast8_t  regid  = *ip.uint8++;
		const uint_fast16_t offset = *ip.uint16++;
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		rsp[regid].uintptr = ( uintptr_t )(rsp + offset);
		DISPATCH();
	}
	exec_lea: { /// u8: opcode | u8: dest | u8: src | i16: offset
		const uint32_t instr = *ip.uint32++;
		const uint_fast8_t dst   = instr & 0xff;
		const uint_fast8_t src   = (instr >> 8) & 0xff;
	...
	exec_ldvar: { /// u8: opcode | u8: regid | u16: index
		const uint_fast8_t  regid = *ip.uint8++;
		const uint_fast16_t index = *ip.uint16++;
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		rsp[regid].uintptr = table_vars->table[index].item;
		DISPATCH();
	}
	...
	exec_ldfn: { /// u8: opcode | u8: regid | u16: index
		...
		const uint_fast16_t index = *ip.uint16++;
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		rsp[regid].uintptr = ( uintptr_t )(&table_funcs->table[index]);
		DISPATCH();
	}
	
	exec_ld1: { /// u8: opcode | u8: dest reg | u8: src reg | i16: offset
		const uint_fast32_t instr = *ip.uint32++;
		const uint_fast8_t  dst   = instr & 0xff;
		const uint_fast8_t  src   = (instr >> 8) & 0xff;
		const int32_t offset = ( int32_t )(instr) >> 16;
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		const uintptr_t mem = rsp[src].uintptr + offset;
		if( (mem - lower_seg) > mem_bnds_diff ) {
	...
	exec_ld2: { /// u8: opcode | u8: dest reg | u8: src reg | i16: offset
		...
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		const uintptr_t mem = rsp[src].uintptr + offset;
		if( (mem+1 - lower_seg) > mem_bnds_diff ) {
			vm->err = TaghaErrBadPtr;
			goto exec_halt;
		} else {
	...
	exec_ld4: { /// u8: opcode | u8: dest reg | u8: src reg | i16: offset
		...
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		const uintptr_t mem = rsp[src].uintptr + offset;
		if( (mem+3 - lower_seg) > mem_bnds_diff ) {
			vm->err = TaghaErrBadPtr;
			goto exec_halt;
		} else {
	...
	exec_ld8: { /// u8: opcode | u8: dest reg | u8: src reg | i16: offset
		...
		union TaghaVal *const restrict rsp = ( union TaghaVal* )(vreg_osp);
		const uintptr_t mem = rsp[src].uintptr + offset;
		if( (mem+7 - lower_seg) > mem_bnds_diff ) {
			vm->err = TaghaErrBadPtr;
			goto exec_halt;
		} else {
	...
	exec_st1: { /// u8: opcode | u8: dest reg | u8: src reg | i16: offset
		...
		const uint_fast8_t  src   = (instr >> 8) & 0xff;
		const int32_t offset = ( int32_t )(instr) >> 16;
		const union TaghaVal *const rsp = ( const union TaghaVal* )(vreg_osp);
...
#	endif
	...
	exec_jmp: { /// u8: opcode | i32: offset
		const int_fast32_t offset = *ip.int32++;
		ip.uint8 += offset;
		DISPATCH();
	}
	...
	exec_call: { /// u8: opcode | u16: index
		const uint_fast16_t index = *ip.uint16++;
		const struct TaghaItem *const func = &table_funcs->table[index - 1];
		...
		if( item==NIL || owner==NIL ) {
			...
			*rsp = (*cfunc)(vm, rsp + 1);
			...
			/**
			 * It's very important to use the owner (callee) module's runtime symbols
			 * instead of the caller module's as the function call system
			 * uses indexes rather than instruction addresses.
			 *
			 * This is so we can segment and maintain some level of safety
			 * with the bytecode execution.
			 *
			 * So when doing an inter-module call, preserve the caller runtime symbols
			 * then restore after call is done.
			 */
			...
			vm->lr = NIL;
			vm->osp = vreg_osp;
			_tagha_module_exec(( union TaghaPtr )(( const void* )(item)), vm);
			vreg_osp = vm->osp;
			_tagha_pop_lr(vm);
			
			vm->funcs = table_funcs;
			vm->vars  = table_vars;
			DISPATCH();
		} else {
			vm->lr   = ( uintptr_t )(ip.uint8);
			ip.uint8 = ( const uint8_t* )(item);
			DISPATCH();
		}
	}
...
#	endif
	...
	exec_setvlen: { /// u8: opcode | u16: vector width
		vm->vec_len = *ip.uint16++;
		DISPATCH();
	}
	exec_setelen: { /// u8: opcode | u8: element width
		vm->elem_len = *ip.uint8++;
		DISPATCH();
	}
