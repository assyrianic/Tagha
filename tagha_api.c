...
inline static uint8_t		*GetFunctionOffsetByName(void *, const char *, bool *);
inline static uint8_t		*GetFunctionOffsetByIndex(void *, size_t, bool *);
...
inline static uint8_t		*GetVariableOffsetByName(void *, const char *, bool *);
inline static uint8_t		*GetVariableOffsetByIndex(void *, size_t, bool *);
inline static size_t		GetVariableIndexByName(void *, const char *, bool *);
inline static const char	*GetVariableNameByIndex(void *, size_t, bool *);

// /*
// void *tagha_dlopen(const char *tbc_module_name);
static void Native_taghadlopen(struct Tagha *sys, void *restrict script, union Value *retval, const size_t args, union Value params[static args])
{
	...
	if( !tbcfile ) {
		printf("tagha_dlopen :: cannot find module '%s'\n", module_name);
		return;
	}
	...
	if( !fseek(tbcfile, 0, SEEK_END) ) {
		...
		if( size == -1LL ) {
			printf("tagha_dlopen :: cannot read file of module '%s'!\n", module_name);
	...
	if( !module ) {
		printf("tagha_dlopen :: failed to load module (\"%s\") into memory!\n", module_name);
		fclose(tbcfile), tbcfile=NULL;
		return;
	}
	...
	if( *(uint16_t *)module != 0xC0DE ) {
		printf("tagha_dlopen :: module (\"%s\") is not a valid TBC script!\n", module_name);
...
// void *tagha_dlsym_func(void *module, const char *symname);
static void Native_taghadlsym(struct Tagha *sys, void *restrict script, union Value *retval, const size_t args, union Value params[static args])
{
	(void)sys; (void)args; (void)script;
	...
	retval->Ptr = GetFunctionOffsetByName(module, symname, NULL);
	//uint8_t *globalsym = GetVariableOffsetByName(module, symname);
}

// void tagha_dlclose(void **moduleref);
static void Native_taghadlclose(struct Tagha *sys, void *restrict script, union Value *retval, const size_t args, union Value params[static args])
{
	puts("Native_taghadlclose :: called");
	(void)sys; (void)args; (void)retval; (void)script;
	void **module = params[0].PtrPtr;
	free(*module), *module = NULL;
	puts("Native_taghadlclose :: returning\n");
}
// */
...
void Tagha_Init(struct Tagha *const restrict vm, void *scripthdr)
{
	if( !vm )
		return;
	...
	*vm = (struct Tagha){0};
	...
	FILE **stdinref = (FILE **)GetVariableOffsetByName(scripthdr, "stdin", NULL);
	if( stdinref )
		*stdinref = stdin;
	
	FILE **stderrref = (FILE **)GetVariableOffsetByName(scripthdr, "stderr", NULL);
	if( stderrref )
		*stderrref = stderr;
		
	FILE **stdoutref = (FILE **)GetVariableOffsetByName(scripthdr, "stderr", NULL);
	if( stdoutref )
		*stdoutref = stdout;
	
	struct NativeInfo dynamic_loading[] = {
		{"tagha_dlopen", Native_taghadlopen},
		{"tagha_dlsym", Native_taghadlsym},
		{"tagha_dlclose", Native_taghadlclose},
		{NULL, NULL}
	};
	Tagha_RegisterNatives(vm, dynamic_loading);
}

void Tagha_Del(struct Tagha *const restrict vm)
{
	if( !vm )
		return;
	*vm = (struct Tagha){0};
...
bool Tagha_RegisterNatives(struct Tagha *const restrict vm, struct NativeInfo natives[])
{
	...
	for( struct NativeInfo *n=natives ; n->NativeCFunc and n->Name ; n++ ) {
		func_addr.Ptr = GetFunctionOffsetByName(vm->ScriptHdr.Ptr, n->Name, &check_native);
...
inline static uint8_t *GetFunctionOffsetByName(void *script, const char *restrict funcname, bool *const restrict isnative)
{
...
inline static size_t GetFunctionIndexByName(void *script, const char *restrict funcname, bool *const restrict isnative)
{
	...
	for( size_t i=0 ; i<funcs ; i++ ) {
		...
		// skip to the next 
		reader.UCharPtr += (stringlen + 8);
	}
	return SIZE_MAX;
}

inline static uint8_t *GetFunctionOffsetByIndex(void *script, const size_t index, bool *const restrict isnative)
{
	if( !script )
		return NULL;
...
inline static uint8_t *GetVariableOffsetByName(void *script, const char *restrict varname, bool *const restrict isnative)
{
...
inline static size_t GetVariableIndexByName(void *script, const char *restrict varname, bool *const restrict isnative)
{
	if( !script or !varname )
		return SIZE_MAX;
...
inline static uint8_t *GetVariableOffsetByIndex(void *script, const size_t index, bool *const restrict isnative)
{
	if( !script )
		return NULL;
...
inline static const char *GetVariableNameByIndex(void *script, const size_t index, bool *const restrict isnative)
{
	if( !script )
		return NULL;
...
#undef INSTR_SET
	...
	exec_lea:; {
		...
		if( addrmode & Immediate ) { // Immediate mode will load a global value
			vm->Regs[regid].Ptr = GetVariableOffsetByIndex(vm->ScriptHdr.Ptr, *vm->Regs[regInstr].UInt64Ptr++, NULL);
		}
		else if( addrmode & Register ) { // Register mode will load a function address which could be a native
			vm->Regs[regid].Ptr = GetFunctionOffsetByIndex(vm->ScriptHdr.Ptr, *vm->Regs[regInstr].UInt64Ptr++, NULL);
		}
		else if( addrmode & RegIndirect ) {
	...
	exec_call:; {
		...
		// call imm
		if( addrmode & Immediate ) {
			int64_t offset = *vm->Regs[regInstr].Int64Ptr++;
			call_addr = vm->Regs[regInstr].UCharPtr + offset;
		}
		// call reg
		else if( addrmode & Register )
			call_addr = vm->Regs[*vm->Regs[regInstr].UCharPtr++].UCharPtr;
		// call [reg+offset]
		else if( addrmode & RegIndirect ) {
			const uint8_t regid = *vm->Regs[regInstr].UCharPtr++;
			const union Value effective_address = (union Value){.UCharPtr = vm->Regs[regid].UCharPtr + *vm->Regs[regInstr].Int32Ptr++};
			if( addrmode & EightBytes )
				call_addr = effective_address.SelfPtr->UCharPtr;
		}
		if( !call_addr ) {
	...
	exec_syscall:; {
		...
		if( addrmode & Immediate ) {
			...
			union Value *nativref = (union Value *)GetFunctionOffsetByIndex(vm->ScriptHdr.Ptr, -1 - *vm->Regs[regInstr].Int64Ptr++, &native_check);
			if( !native_check ) {
		...
		// syscall argcount, reg
			...
			NativeFunc = vm->Regs[*vm->Regs[regInstr].UCharPtr++].VoidFunc;
		}
		// syscall argcount, [reg+offset]
		else if( addrmode & RegIndirect ) {
			const uint8_t regid = *vm->Regs[regInstr].UCharPtr++;
			const union Value effective_address = (union Value){.UCharPtr = vm->Regs[regid].UCharPtr + *vm->Regs[regInstr].Int32Ptr++};
			NativeFunc = effective_address.SelfPtr->VoidFunc;
		}
		...
		(*NativeFunc)(vm, vm->ScriptHdr.Ptr, vm->Regs+regAlaf, argcount, params);
		DISPATCH();
	}
...
int32_t Tagha_RunScript(struct Tagha *const restrict vm, const int32_t argc, char *argv[])
{
	...
	else if( *vm->ScriptHdr.UShortPtr != 0xC0DE ) {
		puts("Tagha_RunScript :: ERROR: Script has invalid main verifier.");
		return -1;
	}
	
	uint8_t *main_offset = GetFunctionOffsetByName(vm->ScriptHdr.Ptr, "main", NULL);
	if( !main_offset ) {
...
int32_t Tagha_CallFunc(struct Tagha *const restrict vm, const char *restrict funcname, const size_t args, union Value values[static args])
{
	...
	uint8_t *func_offset = GetFunctionOffsetByName(vm->ScriptHdr.Ptr, funcname, NULL);
	if( !func_offset ) {
...
{
	return !vm or !varname ? NULL : GetVariableOffsetByName(vm->ScriptHdr.Ptr, varname, NULL);
}
