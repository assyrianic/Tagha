...
def make_header(f, entryaddr:int, memsize:int, stksize:int, nativelen:int):
	f.write(0xC0DE.to_bytes(2, byteorder='little'));
	f.write(entryaddr.to_bytes(4, byteorder='little'));
	f.write(memsize.to_bytes(4, byteorder='little'));
	f.write(stksize.to_bytes(4, byteorder='little'));
	f.write(nativelen.to_bytes(4, byteorder='little'));

def write_header_natives(f, *natives):
	i = 0
	while i<len(natives):
...
with open('pyNative.tbc', 'wb') as bytecode:
	make_header(bytecode, 27, 0, 16, 1);
	write_header_natives(bytecode, 'test');
	bytecode.write(opcodes.pushl.to_bytes(1, byteorder='big'));
	bytecode.write(0x32.to_bytes(4, byteorder='big'));
	bytecode.write(opcodes.pushl.to_bytes(1, byteorder='big'));
	bytecode.write(0x64.to_bytes(4, byteorder='big'));
	bytecode.write(opcodes.pushl.to_bytes(1, byteorder='big'));
	bytecode.write(0x43960000.to_bytes(4, byteorder='big'));
	bytecode.write(opcodes.callnat.to_bytes(1, byteorder='big'));
	bytecode.write(0x0.to_bytes(4, byteorder='big'));
	bytecode.write(0xc.to_bytes(4, byteorder='big'));
	bytecode.write(0x1.to_bytes(4, byteorder='big'));
	bytecode.write(opcodes.halt.to_bytes(1, byteorder='big'));
