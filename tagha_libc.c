...
static void native_get_stk_size(Script_t *script, Param_t params[], Param_t **restrict retval, const uint32_t argc, TaghaVM_t *env)
{
	Script_t *restrict other = params[0].Pointer;
	if( !other )
		puts("script_get_stk_size reported: 'script' is NULL!\n");
...
static void native_script_callfunc(Script_t *script, Param_t params[], Param_t **restrict retval, const uint32_t argc, TaghaVM_t *env)
{
	*retval = NULL;
	...
	env->m_pScript = other;
	Tagha_call_script_func(env, strFunc);
